...
for k in "${!EXTERNAL_APPS[@]}";
do
	# Verify if it is already installed
	if [[ `dpkg -l | grep $k | wc -l` -gt 0 ]]
	then
...
for (( i=0; i<${#SNAP_APPS[@]}; i++ ))
do
	echo -ne "${GREEN}Installing ${NC}${SNAP_APPS[$i]} [    ]"
  snap install ${SNAP_APPS[$i]} &> /dev/null
	PKG_FORMATED=`echo ${SNAP_APPS[$i]} | sed 's/\ \-\-classic//'`
  if [[ `snap list | grep ${PKG_FORMATED} | wc -l` -gt 0 ]]
  then
    echo -e "\b\b\b\b${GREEN}OK ${NC}"
	else
		echo -e "\b\b\b\b\b${RED}FAIL${NC}"
